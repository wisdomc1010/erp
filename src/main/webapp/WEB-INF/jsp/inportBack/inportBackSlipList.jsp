...
<head>
	...
	<script type="text/javascript" src="erp_js/inportBack/inportBackSlipList.js"></script>
</head>
     ...
     <div class="tools">
        ...
        <div>
            <ul class="searchtoolbar">
                <li><a href="inportBack/inportBackSlipAdd"><span><img src="images/t01.png" /></span>添加</a></li>
        ...
        <thead>
                <tr>
			       ...
			       <th width='70px'>出库人</th>
			       <th width='90px'>出库时间</th>
        ...
        <tbody>
            <c:forEach items="${inportBackSlip}" var="i">
                <tr>
                     <td width="30px"><input name="" type="checkbox" value="" /></td>
                     <td id="inportBackId" style="display: none">${i.id}</td>
                     <td><a href="inportBack/showInportBackDetail/${i.id}"><span id="inportBackNum">${i.inportNum}</span></a></td>
                     <td>${i.supplier}</td>
                     <td>${i.sumNumber}</td>
                     <td>
                      	<c:if test="${i.state==0}">未审核</c:if>
                      	<c:if test="${i.state==1}">已审核</c:if>
                      	<c:if test="${i.state==2}">已出库</c:if>
        ...
        <ul class="paginList">
            <c:choose>
                <c:when test="${page.page>=page.totalPage && page.totalPage != 1}">
                           <li class="paginItem"><a href="inportBack/inportBackSlipList/1/*/-1/1"><span class="pagepreEndCurrent"></span></a></li>
                       </c:when>
                <c:otherwise>
            ...
            <c:choose>
                <c:when test="${page.page>1}">
                    <li class="paginItem"><a href="inportBack/inportBackSlipList/1/*/-1/${page.page-1}"><span class="pagepreCurrent"></span></a></li>
                       </c:when>
                <c:otherwise>
            ...
            <c:forEach var="i" begin="1" end="${page.totalPage}">
                <c:choose>
                    <c:when test="${i==page.page}">
                        <li class="paginItem current"><a href="inportBack/inportBackSlipList/1/*/-1/${i}">${i}</a></li>
                    </c:when>
                    <c:otherwise>
                        <li class="paginItem"><a href="inportBack/inportBackSlipList/1/*/-1/${i}">${i}</a></li>
                    </c:otherwise>
                </c:choose>
            </c:forEach>
            ...
            <c:choose>
                <c:when test="${page.page<page.totalPage && page.totalPage!=1}">
                    <li class="paginItem"><a href="inportBack/inportBackSlipList/1/*/-1/${page.page+1}"><span class="pagenxtCurrent"></span></a></li>
                       </c:when>
                <c:otherwise>
            ...
            <c:choose>
                <c:when test="${page.page<page.totalPage}">
                    <li class="paginItem"><a href="inportBack/inportBackSlipList/1/*/-1/${page.totalPage}"><span class="pagenxtEndCurrent"></span></a></li>
                       </c:when>
                <c:otherwise>
