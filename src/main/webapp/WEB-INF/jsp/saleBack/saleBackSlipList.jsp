...
<head>
    ...
    <script type="text/javascript" src="erp_js/saleBack/saleBackSlipList.js"></script>
     ...
     <form:form commandName="page">
        ...
        <div>
            <ul class="searchtoolbar">
                <li><a href="saleBack/saleBackSlipAdd"><span><img src="images/t01.png" /></span>添加</a></li>
            ...
            <tbody>
                <c:forEach items="${saleBackSlip}" var="s">
                    <tr>
                        <td width="30px"><input name="" type="checkbox" value="" /></td>
                        <td id="saleBackId" style="display: none">${s.id}</td>
                        <td><a href="saleBack/showSaleBackDetail/${s.id}"><span id="saleNum">${s.saleNum}</span></a></td>
                        <td>${s.customer}</td>
                        <td>${s.sumNumber}</td>
                        <td>${s.sumPrice}</td>
                        <td>
            ...
            <ul class="paginList">
                <c:choose>
                    <c:when test="${page.page>=page.totalPage && page.totalPage != 1}">
                               <li class="paginItem"><a href="saleBack/saleBackSlipList/1/*/*/-1/1"><span class="pagepreEndCurrent"></span></a></li>
                           </c:when>
                    <c:otherwise>
                ...
                <c:choose>
                    <c:when test="${page.page>1}">
                        <li class="paginItem"><a href="saleBack/saleBackSlipList/1/*/*/-1/${page.page-1}"><span class="pagepreCurrent"></span></a></li>
                           </c:when>
                    <c:otherwise>
                ...
                <c:forEach var="i" begin="1" end="${page.totalPage}">
                    <c:choose>
                        <c:when test="${i==page.page}">
                            <li class="paginItem current"><a href="saleBack/saleBackSlipList/1/*/*/-1/${i}">${i}</a></li>
                        </c:when>
                        <c:otherwise>
                            <li class="paginItem"><a href="saleBack/saleBackSlipList/1/*/*/-1/${i}">${i}</a></li>
                        </c:otherwise>
                    </c:choose>
                </c:forEach>
