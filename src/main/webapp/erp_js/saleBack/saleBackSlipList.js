$(document).ready(function() {
    ...
    $("#edit").click(function() {
        ...
        if (len == 1) {
            item.each(function() {
           	var saleBackId = $(this).parent("td").parent("tr").find("#saleBackId").html();
           	state_flag = searchState(saleBackId);
	      	if(state_flag==0){
	            search_flag = true;
	      	}else{
	      		search_flag = false;
	      	...
	      	if(search_flag){
	      		window.location = window.basePath +	"saleBack/saleBackSlipEdit/" + saleBackId;
	      	}else{
   ...
   $("#delete").click(function() {
           ...
           item.each(function() {
	      	...
	      	if(search_flag){
	      		$.ajax({
	                 type: "get",
	                 async: false,
	                 url: window.basePath +"saleBack/saleBackDelete/" + saleBackId,
	                 success: function(ok) {
	                     if (ok) {
	                         window.confirm("删除操作成功！");
	                         window.location = window.basePath + "saleBack/saleBackSlipList/1/*/*/-1/1";
	                     }
	                 }
	             });
	      	}else{
  	...
  	function searchState(saleBackId){
  		var state_flag = 0; //代表单据状态
  	   	$.ajax({
          type: "get",
          async: false,
          url: window.basePath +"saleBack/saleBackState/" + saleBackId,
          success: function(ok) {
       	   state_flag = ok;
          }
      });
   ...
   $("#updateState").click(function() {
         ...
         if (len == 0) {
             ...
             item.each(function() {
           	  saleBackId = $(this).parent("td").parent( "tr").find("#saleBackId").html();
           	  state_flag = searchState(saleBackId);
           	  if(state_flag==0){
                 	str += saleBackId + ",";
                 	search_flag = true;
             ...
             if(search_flag){
	             ...
	             if (r) {
	                 $.ajax({
	                         url: window.basePath + "saleBack/updateState?",
	                         type: "POST",
	                         dataType: "json",
	                         data: "strArr=" + str,
   ...
   $("#updateBackState").click(function() {
       ...
       if (len == 0) {
           ...
           if(search_flag){
	             ...
	             if (r) {
	                 $.ajax({
                        url: window.basePath + "saleBack/updateBackState?",
                        type: "POST",
                        dataType: "json",
                        data: "strArr=" + str,
                        success : function(data) {
           ...
           item.each(function() {
	      	...
	      	if(search_flag){
	      		$.ajax({
	                 type: "get",
	                 async: false,
	                 url: window.basePath +"saleBack/saleBackReview/" + saleBackId,
           ...
           item.each(function() {
	      	...
	      	if(search_flag){
	      		$.ajax({
	                 type: "get",
	                 async: false,
	                 url: window.basePath +"saleBack/saleBackAccount/" + saleBackId,
	                 success: function(ok) {
	                     if (ok) {
  ...
  //搜索按钮处理
    $("#search").click(function() {
    	...
    	var url = "saleBack/saleBackSlipList/1";
    	//添加客户条件
    	if(customer == ""){
    		url = url +"/*";
    	}else{
