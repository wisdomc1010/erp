$(document).ready(function() {
  ...
  //全选checkbox，选中行
    ...
    $("#edit").click(function() {
        ...
        if (len == 1) {
            item.each(function() {
                var id = $(this).parent("td").parent("tr").find("#supplierId").html();
                var state_flag = false; //代表审核状态是否为已审核
               	$.ajax({
                       type: "get",
                       async: false,
                       url: window.basePath +"supplier/supplierCheckState/" + id,
                       success: function(ok) {
                       	if(ok==1){
               	...
               	if(state_flag){
                    window.location = window.basePath +
                        "supplier/supplierInfoEdit/toEdit/" + id;
                    return true;
               	}
            });
        }
    ...
    $("#updateState").click(function() {
        ...
        if (len == 0) {
            ...
            item.each(function() {
                str += $(this).parent("td").parent("tr").find("#supplierId").html() + ",";
            });
            var r = window.confirm("确定要更新此 " + len + " 条数据吗？");
            if (r) {
                $.ajax({
                    url: window.basePath + "supplier/updateState?",
                    type: "POST",
                    dataType: "json",
                    data: "strArr=" +
                        str,
    ...
    $("#updateUseState").click(function() {
        ...
        if (len == 0) {
            ...
            if (r) {
                $.ajax({
                    url: window.basePath + "supplier/updateUseState?",
                    type: "POST",
                    dataType: "json",
                    data: "strArr=" + str,
                    success : function(data) {
  ...
  //搜索按钮处理
    $("#search").click(function() {
    	...
    	var url = "/supplier/supplierInfoList";
    	//添加客户名称条件
    	if(name == ""){
    		url = url +"/*";
    	}else{
