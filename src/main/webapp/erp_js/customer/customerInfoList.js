$(document).ready(function() {
    ...
    $("#edit").click(function() {
        ...
        if (len == 1) {
            item.each(function() {
                var id = $(this).parent("td").parent("tr").find("#customerId").html();
    ...
    $("#updateState").click(function() {
        ...
        if (len == 0) {
            ...
            item.each(function() {
                str += $(this).parent("td").parent("tr").find("#customerId").html() + ",";
            });
            var r = window.confirm("确定要更新此 " + len + " 条数据吗？");
            if (r) {
                $.ajax({
                    url: window.basePath + "customer/updateState?",
                    type: "POST",
                    dataType: "json",
                    data: "strArr=" + str,
    ...
    $("#updateUseState").click(function() {
        ...
        if (len == 0) {
            ...
            if (r) {
                $.ajax({
                    url: window.basePath + "customer/updateUseState?",
                    type: "POST",
                    dataType: "json",
                    data: "strArr=" + str,
                    success : function(data) {
