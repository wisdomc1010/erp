$(document).ready(function() {
    ...
    $("#edit").click(function() {
        ...
        if (len == 1) {
            item.each(function() {
           	var saleId = $(this).parent("td").parent("tr").find("#saleId").html();
           	state_flag = searchState(saleId);
	      	if(state_flag==0){
	            search_flag = true;
	      	}else{
	      		search_flag = false;
	      	...
	      	if(search_flag){
	      		window.location = window.basePath +	"sale/saleSlipEdit/" + saleId;
	      	}else{
   ...
   $("#delete").click(function() {
           ...
           item.each(function() {
	      	...
	      	if(search_flag){
	      		$.ajax({
	                 type: "get",
	                 async: false,
	                 url: window.basePath +"sale/saleDelete/" + saleId,
	                 success: function(ok) {
	                     if (ok) {
	                         window.confirm("删除操作成功！");
	                         window.location = window.basePath + "sale/saleSlipList/0/*/*/-1/1";
	                     }
	                 }
	             });
	      	}else{
  	...
  	function searchState(saleId){
  		var state_flag = 0; //代表单据状态
  	   	$.ajax({
          type: "get",
          async: false,
          url: window.basePath +"sale/saleState/" + saleId,
          success: function(ok) {
       	   state_flag = ok;
          }
      });
   ...
   $("#updateState").click(function() {
         ...
         if (len == 0) {
             ...
             item.each(function() {
           	  saleId = $(this).parent("td").parent( "tr").find("#saleId").html();
           	  state_flag = searchState(saleId);
           	  if(state_flag==0){
                 	str += saleId + ",";
                 	search_flag = true;
             ...
             if(search_flag){
	             ...
	             if (r) {
	                 $.ajax({
	                         url: window.basePath + "sale/updateState?",
	                         type: "POST",
	                         dataType: "json",
	                         data: "strArr=" + str,
   ...
   $("#updateBackState").click(function() {
       ...
       if (len == 0) {
           ...
           if(search_flag){
	             ...
	             if (r) {
	                 $.ajax({
                        url: window.basePath + "sale/updateBackState?",
                        type: "POST",
                        dataType: "json",
                        data: "strArr=" + str,
                        success : function(data) {
           ...
           item.each(function() {
	      	...
	      	if(search_flag){
	      		$.ajax({
	                 type: "get",
	                 async: false,
	                 url: window.basePath +"sale/saleReview/" + saleId,
           ...
           item.each(function() {
	      	...
	      	if(search_flag){
	      		$.ajax({
	                 type: "get",
	                 async: false,
	                 url: window.basePath +"sale/saleAccount/" + saleId,
	                 success: function(ok) {
	                     if (ok) {
