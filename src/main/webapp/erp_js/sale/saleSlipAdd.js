$(document).ready(function() {
	...
	$("#saveCancel").click(function() {
		if(confirm('确定不保存吗？')){
			parent.location.href = window.basePath + "sale/saleSlipManage";
		}
	});
	...
	$("#goodsSelect").live("change",function(){
		...
		$("#goodsStockShow").empty();
		$("#goodsPriceShow").empty();
		$.ajax({
		   url:window.basePath + "sale/saleStock/" + goodsId,
		   dataType:"json",
		   type:"GET",
		   success: function(data){
			 $("#goodsStockShow").show();
			 $("#goodsStockShow").val(data.goodsNum);
			 $("#goodsPriceShow").show();
	...
	$("#removeDetail").click(function() {
		var item = $("#saleSlipDetail tbody input[type=checkbox]:checked");
		var len = item.length;
		if (len == 0) {
	...
	//保存界面数据
	$("#save").click(function(){
		var mainRows = $("#saleSlipDetail tbody tr").length;
		...
		if(mainRows > 0 ){
			checkNumber($("#saleSlipDetail"));
			if(detailNumber_flag){
				alert('请检查\"销售数量\"是否填写！');
				return false;
			}else{
				$.ajax({
		    		url:window.basePath + "/sale/saleDetailSave",
		    		type:"POST",
		     		dataType:"json",
		     		...
		     		success : function(data) {//data是服务器返回内容
						...
						if(ok){
							alert("保存成功！");
		      			 	parent.location = window.basePath + "/sale/saleSlipManage";
						} else {
							alert(data.fail);
						}
					},
					error : function(error) {
						alert("保存失败！请检查【销售单明细】中数据有效性！");
					}
		    	});
			}
		}
