$(document).ready(function() {
	...
	$("#saveCancel").click(function() {
		if(confirm('确定不保存吗？')){
			parent.location.href = window.basePath + "inport/inportSlipManage";
		}
	});
	...
	$("#save").live("click",function(){
		...
		if(mainRows > 0 ){
			...
			if(detailNumber_flag){
				...
				$.ajax({
		    		url:window.basePath + "inport/inportSlipEdit/toEdit",
		    		type:"POST",
		     		dataType:"json",
		     		data:$("#saveDetail").serialize() + "&inportId=" + inportId + "&inportNum=" + inportNum,
